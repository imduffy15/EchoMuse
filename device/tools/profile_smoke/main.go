...
// the AEC reference actually hear it.
//
// It restores the device on exit — restarting the Android audio services it
// stopped and putting the amp back — so a bring-up run does not leave the
// system silent.
//
...
func main() {
	...
	restore := func() {
		...
		fmt.Println("\nrestored: audio services restarted, amp reset")
	}
	...
	// --- beamformer bypass ---
	fmt.Println("\n== beamformer bypass ==")
	bp := beamformer.NewBypass(prof.Mic.Channels, prof.Mic.WakeChannel)
