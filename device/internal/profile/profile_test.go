...
func TestChannelRolesInRange(t *testing.T) {
	for _, p := range all(t) {
		...
		if p.Mic.WakeChannel < 0 || p.Mic.WakeChannel >= p.Mic.Channels {
			t.Errorf("%s: wake channel %d outside 0..%d",
				p.Name, p.Mic.WakeChannel, p.Mic.Channels-1)
		}
		// The wake channel has to be a microphone. Pointing it at the AEC
		// reference would feed the wake word detector the device's own output.
		if len(p.Mic.MicChannels) > 0 {
			var isMic bool
			for _, ch := range p.Mic.MicChannels {
				if ch == p.Mic.WakeChannel {
					isMic = true
				}
			}
			if !isMic {
				t.Errorf("%s: wake channel %d is not one of the mic channels %v",
					p.Name, p.Mic.WakeChannel, p.Mic.MicChannels)
			}
		}
...
func TestVolumeControlConfigured(t *testing.T) {
	for _, p := range all(t) {
		...
		if p.Volume.Max <= 0 {
			t.Errorf("%s: volume Max = %d", p.Name, p.Volume.Max)
		}
	}
}
...
func TestCapabilitiesMatchHardware(t *testing.T) {
	for _, p := range all(t) {
		...
		if got := has("beamforming"); got != p.Beamforming {
			t.Errorf("%s: beamforming=%v but Beamforming=%v", p.Name, got, p.Beamforming)
		}
	}
}
