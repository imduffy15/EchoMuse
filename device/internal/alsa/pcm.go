...
// which avoids both CGO and the tinyalsa shared library. That matters on
// checkers (Echo Show 5 gen 1): the mic PCM only accepts S24_3LE, a format the
// stock tinycap cannot request, and the whole point of the port is to bypass
// Android's audio HAL rather than link against more of it.
//
...
// Caps reports what a PCM device actually supports, as refined by the driver
...
func Open(cfg Config) (*PCM, error) {
	...
	binary.LittleEndian.PutUint32(sw[24:], boundary) // stop_threshold: free-run
